...
template SerializingMemberNumbers(Classes...) {
    static if (Classes.length == 0)
        ...
        enum SerializingMemberNumbers = Filter!(isPackedField, Classes[0].tupleof).length + SerializingMemberNumbers!(
                Classes[1 .. $]);
}
...
template getFieldName(Type, size_t i) {
    ...
    static assert((is(Unqual!Type == class) || is(Unqual!Type == struct)), "Type must be class or struct: type = " ~ Type
            .stringof);
