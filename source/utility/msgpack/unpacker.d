module utility.msgpack.unpacker;
...
public:
    ...
    /// ditto
    ref Unpacker unpack(T)(ref T value)
     ...
     *  self, i.e. for method chaining.
     */
    ref Unpacker unpack(T, Args...)(ref T object, auto ref Args args)
            if (is(Unqual!T == class)) {
        if (checkNil())
        ...
        if (object is null) {
            static if (Args.length == 0) {
                ...
                throw new MessagePackException("Don't know how to construct class type '" ~ Unqual!T.stringof ~ "' with argument types '" ~ Args
                        .stringof ~ "'.");
            }
        }
        ...
        static if (hasMember!(T, "fromMsgpack")) {
            ...
            if (T.classinfo !is object.classinfo) {
                throw new MessagePackException(
                    "Can't unpack derived class through reference to base class.");
            }
    ...
    /// ditto
    ref Unpacker unpack(T)(ref T object)
...
unittest {
    ...
    { // floating point
        ...
        static if (real.sizeof == double.sizeof || !EnableReal) {
            ...
            Tuple!(float, double, double) test = tuple(cast(float) float.min_normal, cast(double) double.max, cast(
                    real) double.min_normal);
        } else {
            Tuple!(float, double, real) result;
            Tuple!(float, double, real) test = tuple(cast(float) float.min_normal, cast(double) double.max, cast(
                    real) real.min_normal);
        }
    ...
    { // pointer
        ...
        Tuple!(ulong*, long*, double*) result = tuple(&origin.field[0], &origin.field[1], &origin
                .field[2]);
        Tuple!(ulong*, long*, double*) test = tuple(&values.field[0], &values.field[1], &values
                .field[2]);
    ...
    { // container
        ...
        Tuple!(ulong[], double[uint], string, bool[2], char[2]) test
            = tuple([1UL, 2], [3U: 4.0, 5: 6.0, 7: 8.0], "MessagePack is nice!", [
                true, false
        ], "D!");
    ...
    { // user defined
    {
        ...
        }
        {
            ...
            mixin DefinePacker;
            C result, test = new C(ushort.max);
    ...
    { // simple struct and class
    {
...

        {
            static struct SimpleProxy1 {
                ...
                static void serialize(ref Packer p, ref string val) {
                    p.pack(to!uint(val));
                }

                static void deserialize(ref Unpacker u, ref string val) {
                    uint tmp;
                    u.unpack(tmp);
                    val = to!string(tmp);
                }
            ...
            static struct SimpleProxy2 {
                ...
                static void serialize(ref Packer p, ref SysTime val) {
                    p.pack(val.toISOExtString());
                }

                static void deserialize(ref Unpacker u, ref SysTime val) {
                    string tmp;
                    u.unpack(tmp);
                    val = SysTime.fromISOExtString(tmp);
                }
            }
            ...
            static struct SimpleWithProxied2 {
                ...
                static SysTime defaultValue() @property {
                    return SysTime(DateTime(2019, 1, 1, 0, 0, 0));
                }
            }
        ...
        static class SimpleD {
            static struct Proxy {
                ...
                static void serialize(ref Packer p, ref bool val) {
                    p.pack(to!string(val));
                }

                static void serialize(ref Packer p, ref uint val) {
                    p.pack(to!string(val));
                }

                static void serialize(ref Packer p, ref ubyte val) {
                    p.pack(to!string(val));
                }

                static void deserialize(ref Unpacker u, ref bool val) {
                    string tmp;
                    u.unpack(tmp);
                    val = to!bool(tmp);
                }

                static void deserialize(ref Unpacker u, ref uint val) {
                    string tmp;
                    u.unpack(tmp);
                    val = to!uint(tmp);
                }

                static void deserialize(ref Unpacker u, ref ubyte val) {
                    string tmp;
                    u.unpack(tmp);
                    val = to!ubyte(tmp);
                }
