...
struct Value {
        ...
        static if (hasMember!(T, "fromMsgpack")) {
            ...
            if (via.array.length != SerializingMemberNumbers!(Classes))
                throw new MessagePackException(
                    "The number of deserialized object member is mismatched");
        ...
        static if (hasMember!(T, "fromMsgpack")) {
            ...
            static if (isTuple!T) {
                if (via.array.length != T.Types.length)
                    throw new MessagePackException(
                        "The number of deserialized Tuple element is mismatched");
                ...
                if (via.array.length != SerializingMemberNumbers!T)
                    throw new MessagePackException(
                        "The number of deserialized struct member is mismatched");
    ...
    hash_t toHash() const nothrow {
        ...
        case Type.nil:
            return 0;
        case Type.boolean:
            return getHash(&via.boolean);
        case Type.unsigned:
            return getHash(&via.uinteger);
        case Type.signed:
            return getHash(&via.integer);
        case Type.floating:
            return getHash(&via.floating);
        case Type.raw:
            return getHash(&via.raw);
        case Type.ext:
            return getHash(&via.ext);
        case Type.array:
            hash_t ret;
            foreach (elem; via.array)
                ret ^= elem.toHash();
...
JSONValue toJSONValue(in Value val) {
    ...
    case Value.Type.boolean:
        return JSONValue(val.via.boolean);
    case Value.Type.unsigned:
        return JSONValue(val.via.uinteger);
    case Value.Type.signed:
        return JSONValue(val.via.integer);
    case Value.Type.floating:
        return JSONValue(val.via.floating);
    case Value.Type.raw:
        return JSONValue(cast(string)(val.via.raw.idup));
    case Value.Type.ext:
        throw new MessagePackException("Unable to convert ext to json");
...
Value fromJSONValue(in JSONValue val) {
    ...
    case JSONType.uinteger:
        return Value(val.uinteger);
    case JSONType.integer:
        return Value(val.integer);
    case JSONType.float_:
        return Value(val.floating);
    case JSONType.string:
        return Value(cast(ubyte[])(val.str));
    case JSONType.array: {
            Value[] vals;
